    ...
    fun onMessageUpdate(): Boolean {
        ...
        tgClient.send(GetMessage(ticket.telegramId, tgId)) {
            if (it.get().content is MessageText) {
                tgClient.send(EditMessageText(ticket.telegramId, tgId, null, InputMessageText(TdApi.FormattedText(newEvent.message.contentRaw, null), false, false))) {}
            } else {
                tgClient.send(EditMessageCaption(ticket.telegramId, tgId, null, TdApi.FormattedText(newEvent.message.contentRaw, null))) {}
            }
